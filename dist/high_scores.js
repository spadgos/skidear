...
export function getHighScoresWithPlaceholder(score) {
...
export function setHighScores(scores) {
    window.localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(scores.toSorted(byScore).slice(0, MAX_SCORES)));
}
...
export function applyUserInputTo(score, key) {
    if (key === 'Backspace') {
...
export function isValidHighScore(score) {
    return score.name.length === 3 && ALLOWABLE_CHARACTERS.test(score.name);
}
//# sourceMappingURL=high_scores.js.map